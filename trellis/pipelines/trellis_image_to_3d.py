...
class TrellisImageTo3DPipeline(Pipeline):
    ...
    def run_with_style(
        ...
        Args:
            ...
            style_params (dict): Style parameters from a previous generation (from extract_style).
                                If provided, uses these parameters for consistency.
            num_samples (int): The number of samples to generate.
        ...
        Example:
            ...
            outputs1, style = pipeline.run_with_style(image1)
            
            # Apply same style to different images
            outputs2, _ = pipeline.run_with_style(image2, style_params=style)
            outputs3, _ = pipeline.run_with_style(image3, style_params=style)
        """
        # Apply style parameters if provided
        if style_params is not None:
            if seed is None:
                seed = style_params.get('seed', 42)
            # Merge style params with provided params (provided params take precedence)
    ...
    def extract_style(seed: int, sparse_structure_sampler_params: dict = None, slat_sampler_params: dict = None) -> dict:
        ...
        This is a convenience method to create style parameters without running a generation first.
        ...
        Example:
            ...
            style = TrellisImageTo3DPipeline.extract_style(
                seed=1234,
                sparse_structure_sampler_params={"steps": 12, "cfg_strength": 7.5},
                slat_sampler_params={"steps": 12, "cfg_strength": 3.0},
            )
            
            # Apply to multiple generations
            outputs1, _ = pipeline.run_with_style(image1, style_params=style)
            outputs2, _ = pipeline.run_with_style(image2, style_params=style)
        """
        return {
            'seed': seed,
    ...
    def interpolate_styles(style_a: dict, style_b: dict, alpha: float = 0.5) -> dict:
        ...
        Note: Only numeric parameters (like cfg_strength) can be interpolated.
        The seed from style_a is used as the base.
        ...
        return {
            'seed': style_a.get('seed', 42),  # Use seed from style_a
